...
namespace Develappers.BillomatNet.Tests.UnitTests
{
    public class InvoiceServicePaymentsTests : UnitTestBase<InvoiceService>
    {
        ...
        {
            const int id = 8;
            const string expectedUri = "/api/invoice-payments/8";
            ...
            await sut.DeletePaymentAsync(id);
        ...
        public async Task Delete_WithInvalidCredentials_ShouldThrowNotAuthorizedException()
        {
            ...
            const string expectedUri = "/api/invoice-payments/1";
            const int id = 1;
            A.CallTo(() => http.DeleteAsync(new Uri(expectedUri, UriKind.Relative), A<CancellationToken>.Ignored))
                .ThrowsAsync(ExceptionFactory.CreateNotAuthorizedException);

            await Assert.ThrowsAsync<NotAuthorizedException>(() => sut.DeletePaymentAsync(id));
            A.CallTo(() => http.DeleteAsync(new Uri(expectedUri, UriKind.Relative), A<CancellationToken>.Ignored))
                .MustHaveHappenedOnceExactly();
        }
        ...
        public async Task Delete_WithInvalidId_ShouldThrowNotFoundException()
        {
            ...
            await Assert.ThrowsAsync<NotFoundException>(() => sut.DeletePaymentAsync(id));
            A.CallTo(() => http.DeleteAsync(new Uri(expectedUri, UriKind.Relative), A<CancellationToken>.Ignored))
                .MustHaveHappenedOnceExactly();
        }
        ...
        public async Task Delete_WithInvalidInputValue_ShouldThrowArgumentException()
        {
            ...
            await Assert.ThrowsAsync<ArgumentException>(() => sut.DeletePaymentAsync(0));
        }
    }
}
