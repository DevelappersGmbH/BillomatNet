...
namespace Develappers.BillomatNet.Tests.UnitTests
{
    public class ClientServiceTests : UnitTestBase<ClientService>
    {
        private WebException CreateNotFoundException()
        {
            ...
            A.CallTo(() => webResponse.StatusCode).Returns(HttpStatusCode.NotFound);
            return new WebException("", null, WebExceptionStatus.ProtocolError, webResponse);
        }

        private WebException CreateNotAuthorizedException()
        {
            var webResponse = A.Fake<HttpWebResponse>();
            A.CallTo(() => webResponse.StatusCode).Returns(HttpStatusCode.Unauthorized);
            return new WebException("", null, WebExceptionStatus.ProtocolError, webResponse);
        }


        [Fact]
        public async Task GetCustomerById_NotAuthorized()
        {
            const int id = 485054;
            ...
            A.CallTo(() => http.GetAsync(A<Uri>.Ignored, A<CancellationToken>.Ignored))
                .ThrowsAsync(CreateNotAuthorizedException());
        ...
        [Fact]
        public async Task GetCustomerById_WhenExisting()
        {
